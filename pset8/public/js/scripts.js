...
$(function() {
    ...
    var options = {
        center: {lat: 33.8687, lng: -84.3351}, // Atlanta, GA 33.8687	-84.3351
...
function addMarker(place)
{
    ...
    google.maps.event.addListener(marker, "click", function() {
        ...
        // get articles for the place (asynchronously)
        $.getJSON("articles.php", {geo: place.postal_code})
        .done(function(data, textStatus, jqXHR) {

            // if there is no data, no news
            if (data.length === 0)
            {
                showInfo(marker, "Slow news day!");
            }

            // else, build an unordered list of links to articles
            else
            {
                // start ul 
                var ul = "<ul>";
                ...
                // show info window at the marker with content
                showInfo(marker, ul);
            }
        })
    ...
    // remember the marker (so we can easily remove it later)
    markers.push(marker);
}
