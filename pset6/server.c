...
char* indexes(const char* path)
{
    char *html = malloc(sizeof(char *)*(strlen(path)+strlen("index.html")+1));
    char *php = malloc(sizeof(char *)*(strlen(path)+strlen("index.html")+1));
    strcpy(html, path);
    strcpy(php, path);
...
bool load(FILE* file, BYTE** content, size_t* length)
{
    ...
    *length = i;     // Stores the number of bytes
    //printf("Data: %s\n", *content); // Print out content of the page      
    //printf("Length: %zu\n", *length); // Print ouf length of the page
    return true;
}
/**
...
const char* lookup(const char* path)
{
    ...
    if (s == NULL)
    {
        ...
        // strcasecmp overlooks capitalization when looking for a match
        if(!strcasecmp(s, ".css"))
        {
            return "text/css";
        }
        else if(!strcasecmp(s, ".html"))
        {
            return "text/html";
        }
...
{
    // Int n without the CRLF
    int n = strlen(line) -2;
    // Allocate memory for copyofLine, a copy of line
    char* copyofLine = malloc(strlen(line));
    // Copy line into the new variable, copyofLine
    strcpy(copyofLine, line);
    char* method = strtok(copyofLine, " ");
    ...
    // If method is not GET, respond with error 405 and return false
    ...
    // If HTTP-version is not HTTP/1.1, respond with error 505 and return false
    char *http=strstr(line, "HTTP/1.1");
    strncpy(http_version, http, 8);
    //printf("\n* http: %s", http_version);
    if(http_version == NULL || strcmp(http_version, "HTTP/1.1") != 0) 
    {
        error(505);
        return false;
    }
    
    int a = n - strlen(http_version) - 1, j = 0;
    for(int i = 4; i < a; i++)
    {
    ...
    //query[0] is '\0'
    request_target[j]='\0';
    //if request-target does not begin with /, respond with error 501 and return false
    if(request_target[0] !='/')
    {
        error(501);
        return false;
    }
    //printf("\n* request_target: %s", request_target);
    // If substring is absent (even if a ? is present), then query should be "", thereby consuming one byte
    // Set q as a string after "?"
    char *q = strstr(request_target, "?");
    
    // If q is null, query is blank AND abs_path is the entire request_target
    //printf("\n* q: %s", q);
    if(q== NULL)
    {
        query="";
    ...
    {
        char *copyofquery = strstr(q, "?")+1;
        strcpy(query, copyofquery);
    }
    ...
    if(q==NULL)
    {
        int b = strlen(request_target);
        strncpy(abs_path, request_target, b);
        abs_path[b]='\0';
        //printf("\n* abs_path: %s\n\n", abs_path);
        return true;
    }
    ...
    {
        int b = strlen(request_target) - strlen(q);
        strncpy(abs_path, request_target, b);
        abs_path[b]='\0';
        //printf("\n* abs_path: %s\n\n", abs_path);
        return true;
